...
# participants) and four topics ("standpoints"), from text generated in
# repeated trials.
#
# Input: for each design, a subdirectory containing four CSV files
#   similarityMatrix_standpoint0.csv ... similarityMatrix_standpoint3.csv
...
#
# Output: similarity_histogram_by_design_standpoint.png/.pdf saved alongside
# this script.
# ==============================================================================
...
# ---- Assemble the long-format data set for all 12 design x standpoint panels -
...
p <- ggplot(hist_df, aes(x = bin_center, y = display_count, fill = is_zero_bin)) +
  ...
  labs(
    title = "Structured Group Simulation Results - Nuclear Power (Skewed)",
    x = "Cosine Similarity",
    y = "Count"
  ) +
...
# WITHIN a row (so the four standpoints for a given design stay directly
...
# ---- Save the figure -----------------------------------------------------------
ggsave("similarity_nuclear4.png", plot = p,
       width = 12, height = 8, dpi = 300)
ggsave("similarity_nuclear4.pdf", plot = p,
       width = 12, height = 8)

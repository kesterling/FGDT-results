...
#
# This is the 3-topic counterpart to make_mds_figure.R (the 4-topic version).
# Everything is identical except: (1) there are three standpoint columns
# instead of four, (2) it reads from the 3-topic trial_id_* subdirectories,
# and (3) the figure title reflects "3 Options".
#
# For each of the three experimental designs (N = 600, N = 60, N = 6) and each
# of the three topics ("standpoints"), places every trial's text as a point in
...
# make_similarity_figure_3topic.R):
...
# 3-topic data (trial_id_2307512764 / _3661427424 / _8074001077) and
# produces mds_by_design_standpoint_3topic.png/.pdf without errors. Note
# that smacofSym() automatically drops any trial that has zero valid
# (non-zero, non-recoded) pairs with every other trial in its panel, since
# such a trial cannot be placed -- a message is printed for each one
# dropped. If you install 'smacof' fresh, it depends on the CRAN package
# 'weights', which in turn needs 'mice', 'gdata', and 'lme4';
# install.packages("smacof") should pull all of this in automatically.
#
# Output: mds_by_design_standpoint_3topic.png/.pdf saved alongside this script.
# ==============================================================================
...
designs <- tibble::tibble(
  ...
  design_label = factor(c("N = 60 (Rand)", "N = 6 (Strat)", "N = 6 (Rand)"),
                         levels = c("N = 60 (Rand)", "N = 6 (Strat)", "N = 6 (Rand)")),
  n_trials     = c(30L, 50L, 50L)
)

standpoints <- tibble::tibble(
  file_tag          = paste0("standpoint", 0:2),
  standpoint_label  = factor(c("More Strict", "About Right", "Less Strict"),
                              levels = c("More Strict", "About Right", "Less Strict"))
)
...
p <- ggplot(mds_data, aes(x = D1, y = D2)) +
  ...
  labs(
    title = "Structured Group Simulation Results - Gun Control (Stratified)\nMultidimensional Scaling of Cosine Similarities",
    x = "Dimension 1",
    y = "Dimension 2"
  ) +
...
# ---- Save the figure -----------------------------------------------------------
ggsave("mds_guncontrol_strat.png", plot = p, width = 10, height = 9, dpi = 300)
ggsave("mds_guncontrol_strat.pdf", plot = p, width = 10, height = 9)

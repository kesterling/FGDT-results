...
ggplot(figdata, aes(x=figdata[,1], y=figdata[,2], fill=Design)) +
  ...
  scale_fill_brewer(palette = 5) +
  labs(
    title = 'Simulation Results - Gun Control (with Stratification)',
    #subtitle = 'Cohen\'s D',
    x = "Cosine Similarity Distribution"
  ) +
  theme_ridges(font_size = 13, grid = TRUE) + theme(axis.title.y = element_blank())
...
ggplot(figdata, aes(x=figdata[,1], y=figdata[,2], fill=Design)) +
  ...
  scale_fill_brewer(palette = 5) +
  labs(
    title = 'Structured Group Simulation Results - Expand U.S. House',
    #subtitle = 'Cohen\'s D',
    x = "Cosine Similarity Distribution"
  ) +
  theme_ridges(font_size = 13, grid = TRUE) + theme(axis.title.y = element_blank())
...
data <-
  ...
  mutate(maxsim = pmax(similarityColumn1, similarityColumn2, similarityColumn3, similarityColumn4, similarityColumn5, na.rm = TRUE),
         ...
         female = ifelse(gender == "Female", yes = 1, no = 0),
         conservative = ifelse(politics == "Conservative" | politics == "Very conservative", yes = 1, no = 0),
         favor = ifelse(standpointNum == 1, yes = 1, no = 0)
  )
...
figdata1a<-cbind(data$maxsim[data$female==0],data$favor[data$female==0])
figdata1b<-cbind(data$maxsim[data$female==1],data$favor[data$female==1])
figdata2a<-cbind(data$maxsim[data$college==0],data$favor[data$college==0])
figdata2b<-cbind(data$maxsim[data$college==1],data$favor[data$college==1])
...
ggplot(figdata, aes(x=figdata[,1], y=figdata[,3], fill=Position)) +
  ...
  scale_fill_brewer(palette = 5) +
  labs(
    title = 'Bias Audit - Expand U.S. House (simulated data)',
    #subtitle = 'Cohen\'s D',
    x = "Cosine Similarity Distribution"
  ) +
  theme_ridges(font_size = 13, grid = TRUE) + theme(axis.title.y = element_blank())
